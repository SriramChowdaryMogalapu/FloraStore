...
function ProductList() {
    ...
    const handleAddToCart = (plant, index) => {
      dispatch(
        addItem({ name: plant.name, image: plant.image, cost: plant.cost })
      );
      setDisabledButtons({ ...disabledButtons, [index]: true });
    };
  const plantsArray = [
    {
        ...
        {plantsArray.map((category, categoryIndex) => (
          <div key={categoryIndex}>
            <div className="category_heading">
              <h1 className="category_heading_name">{category.category}</h1>
            </div>
            <div className="product-list">
              {category.plants.map((plant, plantIndex) => (
                <div className="product-card" key={plantIndex}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleAddToCart(plant, categoryIndex + "-" + plantIndex)
                    }
                    disabled={disabledButtons[categoryIndex + "-" + plantIndex]}
                  >
                    Add to Cart
                  </button>
                </div>
